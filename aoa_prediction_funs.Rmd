...
lexcat_coefs <- crossling_model_data %>%
  ...
  map_df(function(crossling_lexcat_data) {
    crossling_lexcat_data <- crossling_model_data %>% filter(lexical_category == "Nouns")
    #cat(unique(as.character(crossling_lexcat_data$lexical_category)))
    crossling_model <- lmer(aoa ~ frequency + mlu + num_characters + concreteness + (1 + frequency + mlu + num_characters + concreteness | language),
                            ...
                            control = lmerControl(optimizer = "nloptwrap",
                                                  #boundary.tol = .01,
                                                  optCtrl = list(maxfun = 1e6, tol = 0.01)))
    summary(crossling_model)
