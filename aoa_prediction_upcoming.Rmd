```{r}
aoa_data <- bind_rows(prop_data) %>%
  group_by(language, form, measure, lexical_category, category, uni_lemma, definition) %>%
  summarise(aoa = age[min(which(fit_prop > 0.5))])

uni_aoa <- aoa_data %>%
  group_by(language, form, measure, lexical_category, category, uni_lemma) %>%
  summarise(aoa = min(aoa, na.rm = TRUE))
```
...
instruments <- words %>%
  ...
  distinct()
freqs <- map2(instruments$language, instruments$form,
