...
fit_inst_measure_uni <- function(inst_measure_uni_data) {
  ...
  constants <- inst_measure_uni_data %>%
    ungroup() %>%
    select(language, measure, lexical_category, lexical_class, uni_lemma, words) %>%
    distinct()
...
fit_inst_measure <- function(inst_measure_data) {
  inst_measure_by_uni <- inst_measure_data %>%
    group_by(language, form, measure, lexical_category, lexical_class, uni_lemma,
             age, data_id) %>%
...
aoa_data <- all_prop_data %>%
  select(language, measure, lexical_category, lexical_class, uni_lemma, words, aoa) %>%
  distinct()
...
#feather::write_feather(aoa_data_filtered, "aoa_data.feather")
```
...
#   geom_text(aes(label = sprintf("r² = %.2f", adj_rsq)),
