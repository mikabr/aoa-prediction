...
Get AoA estimates.
```{r aoa_data}
aoa_data <- feather::read_feather("aoa_data.feather")
languages <- unique(aoa_data$language)
```

Read CHILDES data, stem
```{r load_childes}
languages <- tolower(unique(aoa_data$language))

load_childes_lang <- function(language) {
  read_csv(sprintf("predictors/childes/data/childes_%s.csv", 
                                   language)) %>%
    filter(!is.na(word)) %>%
    mutate(stem = get_word_stems(word, language)) %>%
    group_by(stem) %>%
    summarise(mean_sent_length = weighted.mean(mean_sent_length, word_count),
              word_count = sum(word_count),
              final_count = sum(final_count),
              solo_count = sum(solo_count))
}
```

Get frequency estimates.
```{r}
inst_freqs <- function(language) {
...
freqs <- map_df(languages, inst_freqs) %>%
  right_join(words)
...
```{r case_map}
source("predictors/frequency/stemmer.R")
special_cases <- read_csv("predictors/frequency/data/english/english_special_cases.csv",
                          col_names = FALSE)
...
special_case_map <- map_df(1:nrow(special_cases), function(i) {
  ...
  data_frame(item = rep(item, 2 * length(trans_opts)),
             gloss = c(trans_opts, get_word_stems(trans_opts, "english")))
})

pattern_map <- uni_lemmas$uni_lemma %>%
  unique() %>%
  map_df(function(item) {
    options <- item %>% strsplit("/") %>% unlist()
    options <- c(options, get_word_stems(options, "english"))
    trans_opts <- map(options, function(opt) map_chr(transforms, ~.x(opt))) %>%
      unlist() %>% unique()
    data_frame(item = rep(item, 2 * length(trans_opts)),
               gloss = c(trans_opts, get_word_stems(trans_opts, "english"))) %>%
...
Get lexical diversity estimates.
```{r}
# lex_div <- read_csv("predictors/lexical_diversity.csv") %>%
#   rename(gloss = word, count = freq) %>%
#   select(gloss, kl, count)
# 
# item_lex_div <- lex_div %>%
#   mutate(gloss = ifelse(stem(gloss) == "", gloss, stem(gloss))) %>%
#   left_join(case_map) %>%
...
# 
# uni_lex_div <- uni_lemmas %>%
#   left_join(item_lex_div) %>%
#   filter(!is.na(count), !is.na(kl)) %>%
#   mutate(log_kl = log(kl),
#          log_count = log(count))
# 
# uni_lex_div$resid_kl <- lm(log_kl ~ log_count, data = uni_lex_div)$residuals

# ggplot(uni_lex_div, aes(x = resid_kl)) +
#   geom_density()
# 
# ggplot(uni_lex_div, aes(x = log_count, y = resid_kl)) +
#   geom_text(aes(label = uni_lemma))
```

Get MLU estimates.
```{r}
inst_mlu <- function(language) {
...
mlus <- map_df(languages, inst_mlu) %>%
  right_join(words)
#  right_join(freq_words)
...
english_predictors <- c("frequency", "mlu", "concreteness", "num_characters",
                        "valence", "arousal", "babiness") #"resid_kl", 
...
```{r}
speechome <- read_csv("predictors/speechome.csv") %>%
  rename(aofp = aoa)

comp_speechome <- english_data %>%
  left_join(speechome)

#r_freq = −0.18
cor(speechome$aofp, speechome$sln.freq.pre, use = "complete.obs")

# r_MLU = 0.19
cor(speechome$aofp, speechome$s.uttlen.pre, use = "complete.obs")

# r_phones = 0.25
cor(speechome$aofp, speechome$s.cmu.phon, use = "complete.obs")

# r_linguistic = −0.28
cor(speechome$aofp, speechome$srl.topic.KL, use = "complete.obs")

speechome_measures <- c(
  aoa = "aofp",
  frequency = "sln.freq.pre",
  mlu = "s.uttlen.pre",
  num_characters = "s.cmu.phon",
  concreteness = "s.mrc.conc",
  resid_kl = "srl.topic.KL"
)

speechome_cors <- data_frame(measure = names(speechome_measures),
                             speechome_measure = speechome_measures) %>%
  mutate(cor = map2_dbl(measure, speechome_measure,
                        ~cor(comp_speechome[[.x]], comp_speechome[[.y]],
                             use = "complete.obs")),
         cor_aoa = map_dbl(measure,
                           ~cor(comp_speechome[[.x]], comp_speechome$aoa,
                                use = "complete.obs")),
         cor_aofp = map_dbl(speechome_measures,
                            ~cor(comp_speechome[[.x]], comp_speechome$aofp,
                                 use = "complete.obs")))

ggplot(comp_speechome, aes(x = speechome_aoa, y = aoa)) +
  coord_fixed() +
  geom_text(aes(label = uni_lemma, colour = lexical_class)) +
  geom_smooth(method = "lm") +
  scale_colour_solarized()

ggplot(comp_speechome, aes(x = resid_kl, y = srl.topic.KL)) +
  coord_fixed() +
  geom_text(aes(label = uni_lemma, colour = lexical_class)) +
  geom_smooth(method = "lm", colour = "black") +
  scale_colour_solarized(name = "") +
  labs(x = "CHILDES KL", y = "Speechome KL")
```
...
english_na_lemmas <- english_data %>%
  ...
  filter(is.na(value)) %>%
  group_by(uni_lemma) %>%
  summarise(num_na = n(), na_preds = paste(unique(predictor), collapse = ", "))
  
english_model_data_fun <- function(english_data, english_predictors) {
  
  english_model_data <- english_data %>%
    ungroup() %>%
    select_(.dots = c("language", "measure", "lexical_category", "uni_lemma", "aoa", english_predictors)) %>%
...
predictor_cors <- english_predictors %>%
  map_df(~data_frame(predictor = .x,
                     cor = cor(english_model_data$aoa, english_model_data[[.x]])))
...
english_model_data_comp <- english_model_data %>% filter(measure == "understands")
english_model_data_prod <- english_model_data %>% filter(measure == "produces")

english_model_comp <- english_model_fun(english_model_data_comp, english_predictors)

english_model_prod <- english_model_fun(english_model_data_prod, english_predictors)
...
#kl_model <- lm(aoa ~ resid_kl, data = english_model_data)

english_cor <- function(english_predictions) {
  cor(english_predictions$aoa, english_predictions$predicted_aoa)
}
  
english_predictions_comp <- english_predictions_fun(english_model_data_comp,
                                                    english_model_comp)
english_predictions_prod <- english_predictions_fun(english_model_data_prod,
                                                    english_model_prod)

english_coef_comp <- tidy(english_model_comp) %>%
  filter(term != "(Intercept)")

english_coef_prod <- tidy(english_model_prod) %>%
  filter(term != "(Intercept)")

# english_predictors_ordered <- english_predictors_comp %>%
#   factor(levels = english_coef_comp$term[order(abs(english_coef_comp$estimate))])

english_predictor_levels <- english_coef_comp$term[
  order(abs(english_coef_comp$estimate),
        decreasing = TRUE)]
english_coef_comp <- english_coef_comp %>%
  mutate(term = factor(term, levels = english_predictor_levels))
english_coef_prod <- english_coef_prod %>%
  mutate(term = factor(term, levels = english_predictor_levels))
```

```{r, fig.width = 7, fig.height = 7}
ggplot(english_predictions, aes(x = predicted_aoa, y = aoa)) +
  geom_text(aes(label = uni_lemma, colour = lexical_category), cex = 4, show.legend = FALSE) +
...
english_coef <- bind_rows(
  mutate(english_coef_comp, measure = "understands"),
  mutate(english_coef_prod, measure = "produces")
) %>%
  mutate(term = factor(term, levels = rev(levels(term))),
         measure = factor(measure, levels = c("understands", "produces")))

ggplot(english_coef, aes(x = term, y = estimate)) +
  facet_wrap(~measure) +
...
crossling_coef %>%
  ...
  bind_rows(lang_coef) %>%
  mutate(language = factor(language, levels = c(langs, "Cross-Linguistic"))) %>%
  ggplot(aes(x = term, y = abs(estimate), colour = term)) +#fill = term)) +
    facet_wrap(~language, ncol = 4) +
    #geom_bar(stat = "identity") +
    ...
    xlab("") +
    ylab("Coefficient Magnitude\n") +
    theme_tufte(base_family = "Open Sans") +
    theme(axis.text.x = element_text(angle = 45, hjust = 1))
```
