...
For each word, the CDI data yield a trajectory reflecting the number of children that are reported to produce or understand the word at each age covered by the instrument (see Figure 1 for some examples).
...
We computed the number of phonemes in each word in each language based on  phonemeic transcriptions of each word obstained using the eSpeak tool [@duddington2012].
...
A potential concern for comparing coefficient estimates is predictor collinearity. Fortunately, in every language, the highest correlations were between MLU and solo frequency (mean over languages _r = _ `r sprintf("%.2f", pair_cor("solo_frequency", "MLU"))`), as expected given the similarity of these factors, with the rest of the pairwise correlations being no greater than _r =_ `r sprintf("%.2f", max_other_cor)`. More importantly, the variance inflation factor for all the predictors is no greater than `r max_vif`, indicating that multicollinearity among the predictors is low.

<!--; and frequency and number of phonemes (mean over languages _r = _ `r sprintf("%.2f", pair_cor("frequency", "num_phons"))`), a reflection of Zipf's Law [@zipf1935].-->
