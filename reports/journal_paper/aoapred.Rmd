...
short_author: Braginsky et al.

supplements_links: "doi.org/10.1162/opmi_a_00026"
...
```{r setup, child="components/setup.Rmd", cache=FALSE}
```

```{r model_checks, child="components/model_checks.Rmd"}
```
...
```{r extremes_table, child="components/extremes_table.Rmd"}
```
...
typology <- yaml::read_yaml("components/typology.yaml") %>%
...
To examine the substructure of predictor variability, we used hierarchical clustering analysis to find the similarity structure among the pairwise correlations between languages' predictors. The resulting dendrograms are shown in Figure\ \ref{fig:clustering}; these broadly reflect language typology, especially for production data. This result suggests that some language-to-language similarity is captured by the profile of coefficient magnitudes our analysis returns. 
