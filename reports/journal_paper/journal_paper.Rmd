...
uni_model_data <- feather::read_feather(
  "../../aoa_unified/saved_data/uni_model_data.feather"
)
coefs <- feather::read_feather("../../aoa_unified/saved_data/coefs.feather")
...
We use CDI data from Wordbank to estimate the age of acquisition for words across `r length(unique(coefs$language))` languages: `r paste(unique(coefs$language), collapse = ", ")`. We then ask what factors are most important for predicting this age of acquisition. Table \ref{table:lang_stats} gives an overview of our data sources.
...
```{r coefs, fig.width=7, fig.height=5, fig.env='figure*', fig.align='center', dev='cairo_pdf', fig.cap='TODO'}
num_coefs <- length(unique(coefs$term))
ggplot(coefs, aes(x = term, y = estimate)) +
  facet_grid(interaction ~ measure) +
