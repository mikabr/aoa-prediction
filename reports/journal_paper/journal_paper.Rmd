...
Yet although each of these information sources has been reliably demonstrated to affect learning in the constrained context of the laboratory, usually conducted with English-language learners, small-scale experimental studies typically do not tell us either whether these information sources are being used in naturalistic situations or which information sources are most important in predicting the larger-scale dynamics of vocabulary acquisition. How do the various information sources differ in their relative contributions to vocabulary growth across language communities, and how does their influence change over the course of development? 

Our approach to addressing these questions is to use large-scale data on vocabulary development to examine these interactions. In aggregating across a large number of children, idiosyncratic patterns of individual variation are less likely to overpower general patterns of which words are relatively easy or hard to learn, allowing us to investigate what features affect their acquisition. We use age of acquisition (AoA) as our window into this issue, creating predictive models of which words are learned earlier and which are learned later, and why.

Previous work using this approach has revealed that in English, within a lexical category, words that are more frequent in speech to children are likely to be learned earlier [@goodman2008]. And further studies have found evidence for semantic networks [@hills2009], neighborhood density [@stokes2010], iconicity [@perry2015], and linguistic distinctiveness [@roy2015] as additional predictors of AoA, suggesting that they are likely contributors to vocabulary development. But these exciting findings are nevertheless limited in their generality because they used different datasets, focused on different predictors, and almost exclusively analyzed English data. They do not allow comparison of the relative importance of the many relevant factors under consideration  in children learning many different languages.

Here we present analyses based on data from Wordbank ([wordbank.stanford.edu](http://wordbank.stanford.edu)), an open repository of cross-linguistic language development data [@frank2016]. By aggregating administrations of the MacArthur-Bates Communicative Development Inventory [CDI; @fenson2007], a family of parent-report vocabulary checklists, Wordbank provides large-scale vocabulary data based on analogous instruments from more than 60,000 children in more than 20 different language communities. Wordbank is thus a novel resource for richer and more powerful analyses of vocabulary learning over development and across languages.

We integrate estimates of words' acquisition trajectories from Wordbank with characterizations of the word learning environment from the CHILDES database [@macwhinney2000] and elsewhere, a data-integration methodology originated by @goodman2008. Using this approach, we examine interactions between a variety of linguistic, environmental, and conceptual factors. We estimate each word's frequency and mean length of utterance (MLU) based on the set of utterances in CHILDES containing the word. Additionally, we obtain ratings of each word's concreteness, valence, arousal, and relevance to babies from previously collected norms. We use these measures to predict words' acquisition trajectories, assessing the relative contributions of each, as well as how they change over development and interact with lexical category. We use thse analyses to answer two important theoretical questions.
...
First, we ask the extent to which there is consistency across languages in the relative importance of predictors. To take one subsidiary question, are there differences in the importance of grammar-related factors in morphologically more complex languages like Russian and Turkish, compared with simpler ones like English? Differences of this type might be revealing of the degree to which learners face different challenges in learning different languages or in different language environments. Alternatively, consistency may suggest the operation of similar learning contexts, mechanisms, or strategies that are not as dependent on the complexities of phonology, morphology, and syntax in a particular language.
