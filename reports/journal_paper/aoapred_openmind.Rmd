...
```{r langcoefs, fig.width=7, fig.height=4.5, fig.cap="Estimates of coefficients in predicting words' developmental trajectories for all languages and measures. Each point represents a predictor's coefficient in one language, with the large point showing the mean across languages."}
...
```{r lexcatcoefs, fig.width=7, fig.height=4, fig.cap="Estimates of coefficients in predicting words' developmental trajectories for each language, measure, and lexical category."}
...
Word length is the one predictor of acquisition that varied substantially between measures: it is far more predictive for production than comprehension. This difference is likely not attributable to differences in word length among languages -- languages' average word lengths for these items don't differ that much, ranging `r min(lengths$mean)`–`r max(lengths$mean)` in mean and `r min(lengths$median)`–`r max(lengths$median)` in median. Thus, as measured here, length seems to reflect effects of production constraints (i.e., how difficult a word is to say) rather than comprehension constraints (i.e., how difficult it is to store or access). This result may explain why the hierarchical clustering analysis above appears more similar to linguistic typology in production than comprehension, that is, the role of production difficulty may be more similar for more typologically-related languages. 
