...
admins <- get_administration_data(mode = data_mode, original_ids = TRUE) %>%
  select(language, form, sex, age, data_id, longitudinal, source_name,
         original_id) %>%
...
```{r raw_data_save}
# feather::write_feather(bind_rows(raw_data), "raw_data.feather")
...
sample_sizes <- raw_data %>%
  map_df(~.x %>%
           group_by(measure) %>%
           summarise(num_admins = n_distinct(data_id)) %>%
           mutate(language = unique(.x$language)))

# feather::write_feather(sample_sizes, "saved_data/sample_sizes.feather")
```
...
```{r}
kl <- read_csv("predictors/topics//2017-05-19_18_29_49_resid_kl_indiv_childes.csv")

uni_kl <- kl %>%
  mutate(stem = stem(word, "english"),
         language = "English") %>%
  right_join(case_map %>% filter(language == "English")) %>%
  group_by(uni_lemma) %>%
  summarise(kl_mean = mean(mean),
            kl_median = median(median))
```
...
predictors <- c("frequency", "MLU", "final_frequency", "solo_frequency", 
                "kl_mean", "kl_median", "length", "concreteness", "valence",
                "arousal", "babiness")
