...
ggplot(coef_cors, aes(y = term)) +
  facet_grid(measure ~ interaction, scales = "free") +
  #coord_flip() +
  geom_point(aes(x = empirical_coef_var, colour = term), size = 3) +
  geom_errorbarh(aes(xmin = ci_lower_coef_var, xmax = ci_upper_coef_var,
                     x = mean_coef_var),
                 height = 0) +
  scale_fill_solarized(guide = FALSE) +
  ...
  ylab("") +
  scale_x_continuous(name = "Coefficient Variance Across Languages")
```
