...
sample_sizes <- raw_data %>%
  map_df(~.x %>%
           distinct(language, measure, age, data_id) %>%
           count(language, measure, age))

save(sample_sizes, file = "../saved_data/sample_sizes.RData")
```
...
#   mutate(word = if_else(is.na(replacement), uni_lemma, replacement)) %>%
#   select(-replacement) %>%
