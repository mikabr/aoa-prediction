---
title: Predicting word acquisition from lexical factors
...
First, connect to the Wordbank database and pull out the WG data.
```{r}
...
items <- get_item_data(mode = "local") %>%
  filter(type == "word") %>%
  select(item_id, language, form, category, lexical_category, item, uni_lemma) %>%
  mutate(num_item_id = as.numeric(substr(item_id, 6, nchar(item_id))))

get_inst_data <- function(language_items, input_language, input_form) {
  get_instrument_data(input_language, input_form, language_items$item_id,
                      administrations = filter(admins, language == input_language, form == input_form),
                      iteminfo = language_items,
                      mode = "local") %>%
    filter(!is.na(age)) %>%
...
languages <- unlist(map(names(items_by_inst), function(inst) strsplit(inst, "_")[[1]][1]))
forms <- unlist(map(names(items_by_inst), function(inst) strsplit(inst, "_")[[1]][2]))
raw_data <- map3(items_by_inst, languages, forms, get_inst_data) %>%
  bind_rows() %>%
  select(language, form, measure, lexical_category, category, uni_lemma, item, item_id, age, data_id, value)
```
