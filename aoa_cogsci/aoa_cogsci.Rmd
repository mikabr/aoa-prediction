...
print(lang_table, type = "latex", comment = FALSE, include.rownames = FALSE,
      table.placement = "hb", format.args = list(big.mark = ","))
```
...
```{r data, fig.height=6, fig.width=7, fig.env='figure*', fig.align='center', dev='cairo_pdf', fig.cap='Relationship between predictors and AoA for each lexical category in each language.'}
...
```{r coefs, fig.height=3.5, fig.width=7, fig.env='figure*', fig.align='center', dev='cairo_pdf', fig.cap='Estimates of predictor coefficients by language.'}
...
```{r coefs_lexcat, fig.height=1.7, fig.width=7, fig.env='figure*', fig.align='center', dev='cairo_pdf', fig.cap='Estimates of predictor coefficients by lexical category.'}
crossling_coefs %>%
...
We computed the number of characters in each word in each language. While imperfect, this metric of length is highly correlated with number of phonemes and syllables in each word. 
...
Perhaps the most important theoretical challenge in the study of early language is how to connect between the scale of an individual observation or experiment and the broader patterns of acquisition that we observe in large datasets. We have strong theories of how individual learning situations proceed [@frank2009, @mcmurray2012]. Although we may not yet be able to discern the unambiguous signatures of these theories in the aggregate data we have available. Our analyses highlight the importance of aggregating across them, including factors predicted to be predictive by different and even competing theories. Even for English, in which our seven predictors capture the most variance (r^2 = .29), much still remains unexplained. Further, this variance is highly reliable---cross-validation using half the English-speaking to predict ages of acquisition for the other half yield r^2 = .98. While adjudicating between theories is a critical component of the scientific process, it is important not to forget that our working theories are incomplete [@breiman2012]. Unification across these theories may be the other critical step in making progress on understanding language learning [@newell1970].
